...
META: dict[str, dict[str, str]] = {
    ...
    "Финансовая грамотность через IT": {"author": "Олеся", "status": "в работе"},
    "Введение в ООП": {"author": "Игорь", "status": "в работе"},
}

REPO_URL = "https://github.com/talaev-sergey/edu-modules"
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  p.lead {{ color: var(--muted); margin: 0 0 1.25rem; font-size: 1.05rem; }}
  a.repo {{
    color: var(--muted);
    margin: 0 0 3rem;
  ...
  a.repo svg {{ display: block; width: 28px; height: 28px; }}
  .grid {{
    display: grid;
...
<body>
  ...
  <a class="repo" href="{repo_url}" target="_blank" rel="noopener" aria-label="Репозиторий на GitHub" title="Репозиторий на GitHub">
...
def main() -> None:
    ...
    (DIST / "index.html").write_text(
        PAGE.format(cards=cards, repo_url=html.escape(REPO_URL)), encoding="utf-8"
    )
    (DIST / ".nojekyll").touch()
