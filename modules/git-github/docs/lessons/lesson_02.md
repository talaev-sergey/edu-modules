...
- Шаги идут по нарастанию: первые команды выполняем все вместе шаг в шаг, последний шаг ученики проходят почти самостоятельно. Команды вводим в Git Bash.
- Напомнить: вставка в Git Bash — правой кнопкой мыши, а не Ctrl+V.

!!! slide "Создаём папку проекта и открываем в ней Git Bash"
    **Мини-теория:** Git работает внутри папки проекта. Сначала создадим обычную папку там, где ты хранишь свои проекты, а затем откроем в ней терминал Git Bash — для этого у Git есть пункт в меню по правой кнопке мыши.

    1. Открой Проводник Windows: нажми Win+E (или щёлкни по значку жёлтой папки на Панели задач).
    2. Перейди в папку, где ты хранишь свои проекты, — например, «Документы», папку Google Drive или другую, которую укажет тьютор.
    3. Щёлкни правой кнопкой мыши по пустому месту, выбери «Создать», затем «Папку».
    4. Дай папке имя `my-first-repo` (английскими буквами, без пробелов) и нажми Enter.
    5. Открой папку двойным щелчком — внутри она пустая.
    6. Щёлкни правой кнопкой мыши по пустому месту внутри папки. В Windows 11 сначала нажми «Показать дополнительные параметры».
    7. В меню выбери пункт «Open Git Bash here» — откроется чёрное окно терминала.
...
!!! slide "Превращаем папку в репозиторий (git init)"
    **Мини-теория:** команда `git init` создаёт внутри папки скрытую папку `.git` — с этого момента Git начинает следить за проектом. Делать это нужно только один раз для каждого проекта.
    ...
    1. В окне Git Bash набери команду `git init` и нажми Enter.
    2. Прочитай сообщение: Git напишет, что создан пустой репозиторий (`Initialized empty Git repository`).
    3. Набери команду `git status` и нажми Enter, чтобы проверить состояние нового репозитория.
...
!!! slide "Создаём файл и смотрим состояние (git status)"
    **Мини-теория:** команда `git status` — главный помощник новичка. Она показывает, какие файлы Git ещё не отслеживает. Новый файл Git считает неотслеживаемым (untracked) и подсвечивает красным.
    ...
    1. Не закрывая Git Bash, вернись в папку `my-first-repo` в Проводнике.
    2. Создай в ней текстовый файл: правая кнопка мыши, «Создать», «Текстовый документ».
    3. Назови файл `about.txt`. Важно: имя должно заканчиваться на `.txt`, а не `.txt.txt`.
    4. Открой файл двойным щелчком, напиши внутри одну строку о себе, например: `Меня зовут Аня, мне 13 лет`. Сохрани файл (Ctrl+S) и закрой.
    5. Вернись в Git Bash, набери команду `git status` и нажми Enter.
    6. Найди в выводе имя файла `about.txt` в разделе Untracked files — это значит, что Git увидел новый файл, но пока его не сохраняет.
...
!!! slide "Добавляем файл в индекс (git add)"
    **Мини-теория:** команда `git add` складывает файл в индекс — готовит его к сохранению. Это как положить вещь в коробку для посылки. После `add` файл в `git status` станет зелёным (Changes to be committed).
    ...
    1. В Git Bash набери команду `git add about.txt` и нажми Enter.
    2. Набери команду `git status` и нажми Enter ещё раз.
    3. Сравни вывод с прошлым разом: теперь файл `about.txt` находится в разделе Changes to be committed — он готов к коммиту.
...
!!! slide "Делаем первый коммит и смотрим историю (git commit, git log)"
    **Мини-теория:** команда `git commit` сохраняет снимок проекта. После `-m` в кавычках пишут короткое описание, что изменилось. Команда `git log` показывает список всех коммитов — историю проекта.
    ...
    1. В Git Bash набери команду одной строкой `git commit -m "Добавил файл about.txt"` и нажми Enter.
    2. Прочитай сообщение: Git напишет, что создан 1 коммит и изменён 1 файл.
    3. Набери команду `git status` — теперь Git сообщает, что сохранять нечего (`nothing to commit, working tree clean`).
    4. Набери команду `git log` и нажми Enter — ты увидишь свой первый коммит: его подпись, твоё имя, дату и описание.
...
!!! warning "Важно"
    Коммит сохраняет только то, что лежит в индексе. Если изменить файл и забыть `git add`, в коммит попадёт не новая правка, а та версия, что лежала в индексе раньше. Свежие изменения останутся несохранёнными. Поэтому порядок всегда такой: сначала `git add`, потом `git commit`. Если коммитить нечего (всё уже сохранено), Git так и напишет: `nothing to commit`.

!!! slide "Второй коммит своими руками (делаешь сам)"
    **Мини-теория:** каждое новое сохранение проходит тот же путь: изменил файл → `git add` → `git commit`. Теперь пройди его сам, подглядывая в свои записи в блокноте, а не в проектор.

    1. Открой файл `about.txt` и допиши вторую строку, например: `Я учусь работать с Git`. Сохрани и закрой файл.
    2. В Git Bash командой `git status` проверь, что Git увидел изменение (файл станет modified).
    3. Командой `git add about.txt` добавь файл в индекс.
    4. Командой `git commit -m "Дописал вторую строку"` сделай второй коммит.
    5. Командой `git log` посмотри историю — теперь в ней должно быть два коммита.
