...
!!! slide "Блок 1. Готовим калькулятор: три операции — три коммита"
    **Мини-теория:** чтобы было что смотреть и отменять, нужна история из нескольких коммитов. Начнём собирать консольный калькулятор и будем добавлять операции по одной, сохраняя каждую отдельным коммитом — так же, как делают программисты.
    ...
    1. Откройте Проводник Windows (Win+E), перейдите в папку для проектов и создайте новую папку с именем `my-first-app` (английскими буквами, без пробелов).
    2. Откройте VS Code. Выберите в меню File → Open Folder и откройте свою папку `my-first-app` — слева появится её содержимое.
    3. Откройте встроенный терминал: в меню Terminal выберите New Terminal (или нажмите Ctrl+Ё). Внизу появится строка терминала — она уже находится внутри папки проекта.
    4. В терминале наберите команду `git init` и нажмите Enter.
    5. Создайте файл: щёлкните по значку «новый файл» рядом с именем папки слева и назовите его `calc.py`.
    6. Напишите первую операцию — сложение — и сохраните файл (Ctrl+S):
    ...
    7. В терминале сделайте первый коммит: `git add calc.py`, затем `git commit -m "Добавил функцию сложения"`.
    8. Допишите вторую операцию — вычитание — и сохраните файл:
    ...
    9. В терминале сделайте второй коммит: `git add calc.py`, затем `git commit -m "Добавил функцию вычитания"`.
    10. Допишите третью операцию — умножение — и сохраните файл:
    ...
    11. В терминале сделайте третий коммит: `git add calc.py`, затем `git commit -m "Добавил функцию умножения"`.
...
!!! slide "Блок 2. Смотрим историю: git log и git show"
    **Мини-теория:** команда `git log` показывает всю ленту коммитов. С флагом `--oneline` она выводит каждый коммит одной короткой строкой — так историю удобнее листать. В начале каждой строки стоит короткий хеш — это «номер» коммита, он нам пригодится дальше. Команда `git show` открывает один коммит подробно и показывает, какие строки кода в нём изменились.
    ...
    1. В терминале наберите команду `git log --oneline` и нажмите Enter. Вы увидите три строки — три операции калькулятора, у каждого коммита свой короткий хеш в начале.
    2. Посмотрите свой самый свежий коммит подробно: наберите `git show` и нажмите Enter. Зелёным показана добавленная функция умножения.
...
!!! slide "Блок 3. Нашли ошибку до коммита: git diff"
    **Мини-теория:** пока изменения не добавлены в индекс командой `git add`, их можно увидеть командой `git diff` — она показывает построчно, что именно поменялось по сравнению с последним коммитом. Программисты так проверяют себя перед сохранением: видно ровно то, что собираешься закоммитить, и можно вовремя заметить ошибку.
    ...
    1. Начните добавлять четвёртую операцию — деление, но с ошибкой: в `return` стоит плюс вместо деления. Сохраните файл, но **НЕ** делайте `git add`:
    ...
    2. В терминале наберите команду `git diff` и нажмите Enter.
    3. Найдите свой новый код — добавленные строки помечены зелёным и знаком плюс в начале. Видно, что в делении по ошибке стоит `+`, а не `/`.

!!! warning "Заметка про git diff"
    Иногда предыдущая последняя строка показана сразу удалённой и добавленной, хотя её не меняли. Так бывает, если в конце файла нет пустой строки: дописав новую строку, вы добавили к предыдущей невидимый перевод строки, и Git отметил её как изменённую. Это не ошибка — обращайте внимание только на по-настоящему добавленные строки кода.

!!! note "Ожидаемый результат"
    `git diff` показывает добавленную функцию деления с ошибкой (`+` вместо `/`), пока не закоммиченную.

!!! slide "Блок 4. Отменяем несохранённую ошибку: git restore"
    **Мини-теория:** ошибку мы заметили вовремя — она ещё не закоммичена. Команда `git restore` вернёт файл к виду из последнего коммита, и неудачная функция исчезнет. Внимание: несохранённые изменения при этом пропадут безвозвратно — `restore` их стирает, отменить это нельзя. Поэтому сначала всегда смотрим `git diff`, а потом решаем.
    ...
    1. Убедитесь, что ошибочную функцию деления вы только дописали и не делали `git add`.
    2. В терминале наберите команду `git restore calc.py` и нажмите Enter.
    3. Посмотрите `calc.py` в редакторе — ошибочной функции деления больше нет, остались три рабочие функции.
    4. Проверьте состояние командой `git status` — Git сообщит, что менять нечего (`working tree clean`).
...
!!! slide "Блок 5. Ошибка уже в коммите: git revert"
    **Мини-теория:** иногда ошибку замечают не сразу, и она успевает попасть в коммит. Стереть такой коммит из истории нельзя и не нужно — есть `git revert`. Он не удаляет ошибочный коммит, а создаёт новый коммит, который отменяет его изменения. История остаётся честной и полной: видно и где ошиблись, и как исправили. Это самый безопасный способ отмены, его используют в командной работе.
    ...
    1. Снова напишите деление — и снова с ошибкой (на этот раз умножение вместо деления), но теперь сохраните её. Сохраните файл:
    ...
    2. В терминале сделайте коммит: `git add calc.py`, затем `git commit -m "Добавил функцию деления"`.
    3. Посмотрите историю: `git log --oneline`. Скопируйте короткий хеш самого верхнего коммита «Добавил функцию деления» (выделить мышью, скопировать — Ctrl+C).
    4. Заметили ошибку: в делении стоит `*` вместо `/`. Коммит уже сделан, поэтому отменяем безопасно: `git revert <хеш> --no-edit` — вместо `<хеш>` вставьте скопированный хеш (Ctrl+V).
    5. Посмотрите `calc.py`: ошибочная функция деления исчезла — её убрал коммит-отмена.
    6. Посмотрите `git log --oneline`: в истории теперь и «Добавил функцию деления», и коммит-отмена `Revert`. История сохранилась полностью.
    7. Теперь напишите деление правильно и сохраните файл:
    ...
    8. В терминале сделайте коммит: `git add calc.py`, затем `git commit -m "Исправил функцию деления"`.
...
!!! slide "Блок 6. Временный переход в прошлое: git checkout (делаешь сам)"
    **Мини-теория:** чтобы заглянуть в раннюю версию калькулятора, есть команда `git checkout <хеш>`. Она переносит рабочую папку в состояние выбранного коммита — код становится таким, каким был тогда. Это безопасный просмотр: ничего не теряется. Чтобы вернуться в настоящее, есть короткая команда `git checkout -` (минус) — она возвращает на ту ветку, где ты был. Пройдите блок сами, подглядывая в записи в блокноте, а не в проектор.

    1. Посмотрите историю командой `git log --oneline` и скопируйте короткий хеш самого первого коммита «Добавил функцию сложения» (он в самом низу списка).
    2. Перенеситесь в прошлое: наберите `git checkout <хеш>` со скопированным хешем и нажмите Enter. Git напишет предупреждение про `detached HEAD` — это нормально, так и должно быть.
    3. Посмотрите `calc.py` в редакторе: в нём снова только одна функция — сложение. Вы в самой первой версии калькулятора.
    4. Вернитесь в настоящее короткой командой: `git checkout -` (минус) и нажмите Enter.
    5. Посмотрите `calc.py` ещё раз — вернулись все функции, калькулятор снова в актуальном виде.
...
- Ключевое различие: `restore` — когда ошибка ещё не закоммичена, `revert` — когда уже закоммичена. Именно это и обыгрывается в блоках 4 и 5 на одной и той же функции деления.
