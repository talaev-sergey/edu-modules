...
!!! slide "Что такое Git"
    Git — это программа, которая следит за изменениями в папке с проектом. Она запоминает каждое сохранение (его называют «коммит»), позволяет вернуться к любой прошлой версии и показывает, что именно поменялось. Git работает через команды, которые мы набираем в специальном окне — терминале. Это как машина времени для твоих файлов.
...
!!! slide "Скачиваем установщик Git"
    **Мини-теория:** программы для Windows скачивают в виде файла-установщика. Git берут только с официального сайта git-scm.com — так мы не поймаем вирус с поддельного сайта.
    ...
    1. Открой браузер и в адресной строке введи: `git-scm.com`
    2. На главной странице нажми кнопку «Download for Windows».
    3. Выбери «64-bit Git for Windows Setup» — начнётся скачивание файла.
    4. Внизу окна браузера или в папке «Загрузки» найди скачанный файл (его имя начинается со слова Git и заканчивается на .exe).

    ![Страница загрузки git-scm.com: ссылка «64-bit Git for Windows Setup»](../assets/images/lesson_01_git_download.webp)
...
!!! slide "Устанавливаем Git"
    **Мини-теория:** установщик задаёт много вопросов, но новичку менять ничего не нужно — стандартные настройки подходят. Нам важно просто несколько раз нажать «Next» и затем «Install».
    ...
    1. Дважды щёлкни по скачанному файлу установщика Git.
    2. Если Windows спросит «Разрешить этому приложению вносить изменения?» (окно UAC) — нажми «Да».
    3. Во всех окнах установщика нажимай кнопку «Next», не меняя настройки.
    4. На последнем шаге нажми «Install» и дождись окончания установки.
    5. Нажми «Finish». Если стоит галочка про заметки о версии — её можно снять.
...
!!! slide "Открываем Git Bash и проверяем установку"
    **Мини-теория:** вместе с Git устанавливается терминал Git Bash. Команда `git -v` показывает номер установленной версии — это самый простой способ проверить, что Git работает.
    ...
    1. Нажми кнопку «Пуск» и начни печатать: Git Bash.
    2. В списке найди приложение «Git Bash» и открой его — появится чёрное окно терминала.
    3. Набери команду: `git -v` и нажми Enter.
    4. Прочитай ответ — в строке должно появиться слово git и номер версии (например, `git version 2.45.0`).
...
!!! slide "Первая настройка Git: имя и e-mail"
    **Мини-теория:** Git подписывает каждое сохранение именем автора, поэтому ему нужно один раз сообщить, кто ты. Эти данные настраиваются командой `git config` и потом используются во всех проектах.

    1. В Git Bash набери команду (вместо Ivan Petrov впиши своё имя латиницей в кавычках) и нажми Enter:

        ```bash
        git config --global user.name "Ivan Petrov"
        ```

    2. Следующей командой задай e-mail (впиши свой e-mail) и нажми Enter:

        ```bash
        git config --global user.email "ivan@example.com"
        ```

    3. Проверь настройки командой:

        ```bash
        git config -l
        ```

    4. Найди в списке строки user.name и user.email — в них должны быть твои данные.
...
!!! slide "Устанавливаем редактор кода VS Code"
    **Мини-теория:** файлы проекта удобно открывать и редактировать в специальном редакторе. Весь модуль мы будем работать в VS Code — это бесплатный редактор от Microsoft.
    ...
    1. Открой браузер и перейди на сайт: `code.visualstudio.com`
    2. Нажми большую кнопку «Download for Windows».
    3. Запусти скачанный установщик, в окне UAC нажми «Да».
    4. Прими условия лицензии, затем нажимай «Next», а в конце — «Install».
    5. После установки нажми «Finish» — VS Code откроется. Закрой вкладку приветствия.
...
!!! note "Что сделать тьютору"
    Перед основным заданием откройте этот квиз на компьютере каждого ученика. Это короткая проверка теории урока: что такое контроль версий, кто и когда создал Git, что такое коммит и репозиторий, а также команды `git -v` и `git config`. Ученик отвечает на все вопросы, при ошибке под вопросом появляется правильный ответ, а справа выставляется итоговая оценка за квиз.
...
- Установка Git: окно UAC и обилие экранов установщика пугают новичков — подробнее во врезке «Где чаще всего теряются» в практической части (UAC — «Да», в установщике — просто «Next»).
...
- Поиграй с терминалом Git Bash: введи команду `cal` — появится календарь текущего месяца, а команда `date` покажет точные дату и время. Найди в календаре день своего рождения.
- Сделай сообщения Git цветными: выполни команду `git config --global color.ui auto`, затем проверь настройки командой `git config -l` и найди новую строку `color.ui` — теперь подсказки Git будут раскрашиваться.
