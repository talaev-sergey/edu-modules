...
!!! slide "Ветка — это отдельная дорожка для работы"
    ...
    ![Граф веток: master с коммитами сложение-вычитание-умножение, от последнего отходит ветка feature-power с коммитом степень — текущий коммит и ветка подсвечены, HEAD указывает на feature-power](../assets/images/lesson_04_branch_map.webp)
...
!!! note "Заметка про имя основной ветки"
    Дальше в заданиях основная ветка называется `master`. Если у тебя `git branch` показал `main` — везде вместо `master` пиши `main`. Имя зависит только от версии Git, на работу с ветками это не влияет.
...
!!! warning "Частая ошибка"
    Если Git при `git switch` пишет, что есть несохранённые изменения и переключиться нельзя — значит, ты не закоммитил правки. Сделай `git add` и `git commit`, потом переключайтесь.
...
!!! tip "Эксперимент: а можно ли удалить саму основную ветку?"
    Сейчас ты стоишь на `master` — попробуем удалить её и посмотрим, что ответит Git. Это безопасно: Git не даст совершить ошибку.
...
#### Интерактивный тренажёр: ветки

Прежде чем пройти весь путь с веткой руками в терминале, разомнитесь на тренажёре. Откройте его на компьютере у каждого ученика и проиграйте несколько сценариев работы с ветками: создавайте ветки, делайте в них коммиты, переключайтесь и сливайте — следите, как на графе двигаются указатели веток и `HEAD`, а в строке «📄 calc.py» меняется набор функций при переходе между ветками.

!!! note "Что сказать ученикам (для тьютора)"
    В тренажёре уже собран знакомый пример — проект-калькулятор (`add`, `subtract`, `multiply`) на ветке `master`, как папка `my-first-app` к этому уроку. Проиграйте вместе с классом разные сценарии, проговаривая вслух каждый шаг и читая подсказки в «терминале» тренажёра:

    - **Простое слияние (fast-forward).** Создайте ветку `feature-power`, сделайте в ней коммит, вернитесь на `master` и влейте ветку — Git напишет `Fast-forward` и просто передвинет указатель `master` вперёд, без отдельного коммита слияния.
    - **Настоящее слияние.** Сделайте коммит в `feature-square`, затем ещё один коммит на самой `master` — ветки разойдутся. Теперь слияние создаст особый **коммит слияния** с двумя родителями; на графе видно, как две дорожки сходятся в одну.
    - **Защита Git.** Покажите, что Git бережёт работу: попробуйте переключиться или слить ветку с несохранёнными правками (кнопка «Изменить calc.py», но без коммита) и удалить ещё не влитую ветку — тренажёр выведет те же ошибки, что и настоящий Git.

    Это симуляция, а не настоящий терминал: команды по-настоящему не выполняются, имена функций и тексты коммитов подставляются автоматически. Те же шаги ученики повторят руками в задании ниже.

**Короткие задания на тренажёре** (пусть каждый ученик пройдёт сам):

1. Заведи ветку `feature-power`, сделай в ней коммит и влей её в `master` так, чтобы получить надпись `Fast-forward`.
2. Добейся настоящего **коммита слияния**: сделай коммит в новой ветке `feature-square`, потом отдельный коммит на `master`, и только затем слей их.
3. Изменив `calc.py`, но не закоммитив, попробуй переключиться на другую ветку — прочитай, что ответит Git, и исправь ситуацию.
4. Удали влитую ветку, а потом попробуй удалить ещё не влитую — сравни, как Git ведёт себя в этих двух случаях.
